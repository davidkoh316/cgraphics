...
bool video_on = false; // CHANGE TO "TRUE" TO RECORD VIDEO
int frame_num = 0;
...
void RayWindow::DisplayFunction(void){
    ...
    if (video_on)
    {
        Image32 image = Image32();
        stringstream str;

        if (frame_num < 10) {str << "0000";}
        else if (frame_num < 100) {str << "000";}
        else if (frame_num < 1000) {str << "00";}
        else if (frame_num < 10000) {str << "0";}

        str << frame_num; //frame_num is initially 0
        str << ".jpeg";
        cout << str.str() << "\n";

        std::string temp = str.str();

        char* enter = new char[temp.size() + 1];
        std::copy(temp.begin(), temp.end(), enter);
        enter[temp.size()] = '\0';

        TakeSnapshot(image);
        image.WriteImage(enter);
